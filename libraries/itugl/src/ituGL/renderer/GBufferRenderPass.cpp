...
{
    //7.4: Depth: Set the min and magfilter as nearest
    ...
    //7.2: Albedo: Bind the newly created texture, set the image, and the min and magfilter as nearest
    m_albedoTexture = std::make_shared<Texture2DObject>();
	m_albedoTexture->Bind();
	m_albedoTexture->SetImage(0, width, height, TextureObject::FormatRGB, TextureObject::InternalFormatRGB);
    ...
    //7.3: Normal: Bind the newly created texture, set the image and the min and magfilter as nearest
    m_normalTexture = std::make_shared<Texture2DObject>();
	m_normalTexture->Bind();
    m_normalTexture->SetImage(0, width, height, TextureObject::FormatRG, TextureObject::InternalFormatRG16);
    ...
    //7.5: Others: Bind the newly created texture, set the image and the min and magfilter as nearest
